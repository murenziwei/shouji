...
Page({
  ...
  onLoad: function (options) {
    ...
    wx.request({
      url: 'https://dv-sapi.chuangsiboai.com/Api/Image/getAllmodelType',
      data: {
        openid: "222222222"
      },
      method: "POST",
      ...
      success(res) {
        ...
        let mode = res.data;
        self.data.basephonemodel = mode;
        for (let i = 0; i < mode.AllBrand.data.length; i++) {
          self.data.mode_phone.push(mode.AllBrand.data[i].brand_name);
        }
        for (let i = 0; i < mode.OneList.data.length; i++) {
          self.data.phone_type.push(mode.OneList.data[i].model_code);
        }

        self.data.selectmodel[0] = self.data.mode_phone;
        self.data.selectmodel[1] = self.data.phone_type;

        self.setData({
          selectmodel: self.data.selectmodel
        })

        self.data.basephoneid = self.data.basephonemodel.AllBrand.data[0].id;
        self.data.basenamevale = self.data.basephonemodel.OneList.data[0].model_code;
    ...
    // 默认规格
    wx.request({
      url: 'https://dv-sapi.chuangsiboai.com/Api/Format/GetGoodsFormat', //
      data: {
        id: 1
      },
      method: "POST",
  ...
  onReady: function () {
    ...
    console.log(this.data.phonetype,phonetype,"我靠");
    ...
    console.log(model);
    wx.request({
      url: 'https://dv-sapi.chuangsiboai.com/Api/Image/getOneModelCode', //
      data: {
        model_code: model,
        brand: brand
      },
  ...
  kuodastart:function(e){
    ...
    twoPoint.x2 = e.touches[0].pageX * 2;
    twoPoint.y2 = e.touches[0].pageY * 2;

    this.data.twoPoint=twoPoint;
    this.data.scalePoint=twoPoint;
  },
  kuodamove:function(e){

    let twoPoint=this.data.twoPoint;
    let sPoint=this.data.scalePoint;
    var that=this;

    var preTwoPoint = JSON.parse(JSON.stringify(twoPoint));

    // console.log(e);
    var xJ2 = e.touches[0].pageX * 2, yJ2 = e.touches[0].pageY * 2;
    if(xJ2>sPoint.x1){
      var isadd=1;
    }else if(xJ2<sPoint.x1){
      var isadd=-1;
    }
    if(xJ2!=sPoint.x1){

      twoPoint.x1 = sPoint.x1
      twoPoint.y1 = sPoint.y1
      twoPoint.x2 = e.touches[0].pageX * 2
      twoPoint.y2 = e.touches[0].pageY * 2
      ...
      // msg: '缩放',
      console.log((curDistance - preDistance), "point");
      let scale = that.data.list[that.data.index].scale + (curDistance - preDistance) * 0.004 * isadd;
      console.log(scale);
      if (scale >= 0.2) {
      ...
      success(res) {
        ...
        wx.getImageInfo({
          ...
          success(res) {
            ...
            wx.uploadFile({
              url: 'https://dv-sapi.chuangsiboai.com/Api/Image/uploadFile',
  ...
  rloadphonetype(event) {
    ...
    if (select.column == 0) {
      ...
      wx.request({
        url: 'https://dv-sapi.chuangsiboai.com/Api/Image/getOneModelType', //
        data: {
        ...
        success(res) {
          ...
          wx.request({
            url: 'https://dv-sapi.chuangsiboai.com/Api/Image/getOneModelCodeId', //
            data: {
    ...
    var setT=setTimeout(()=>{wx.hideLoading();clearTimeout(setT)},5000)
  ...
  selectsavebtn() {
    ...
    if (status) {
      ...
      wx.request({
        url: 'https://dv-sapi.chuangsiboai.com/Api/Format/getFormatPrice',
        data: {
          "goods_id": 1,
          "value_name": newstr
        },
    ...
    if (this.data.autoSelectStatus == true && this.data.model_code != "未识别") {
      ...
      if (this.data.value_name == '') {
        ...
        if (this.data.list.length > 0) {
          ...
          console.log(JSON.stringify(self.data.list))
          wx.request({
            url: 'https://dv-sapi.chuangsiboai.com/Api/Image/getPictrue',
            data: {
              picturelist: JSON.stringify(self.data.list),
              model_code: self.data.model_code, //假的
            ...
            success(res) {
              ...
              if(res.data.info){
                ...
                wx.request({
                  url: 'https://dv-sapi.chuangsiboai.com/api/Image/booking_string',
                  data: {
                    goods_id: 1,
                ...
                wx.showToast({title:res.data.error})
              }
  ...
  bindchangebtn(event) {
    ...
    var rep = this.data.phonebasetype;
    if (rep) {
      rep.brand_id = brand_id;
    } else {
      rep = { brand_id: brand_id }
    }
    this.data.phonebasetype = rep;
